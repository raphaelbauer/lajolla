...
public class PaperSaraTest extends TestCase {
   ...
   *
   * The present results quantify the increase in accuracy of RNA structure
   * alignment compared to existing methods such as ARTS. Although such
   * differences in accuracy may seem small, our benchmark indicates that in
   * average the observed differences are statistically significant at the 95%
   * confidence level. For example, the structural alignment of a sarcin/ricin
   * domain 28S rRNA (PDB code 1q96 chain A) with a 5S Ribosomal RNA (PDB code
   * 1un6 chain E) results in 50% more base pairs and 15.7% more nucleotides
   * aligned by SARA compared to the ARTS alignment (Fig. 6). The difference in
   * such alignment are due to a more accurate superposition of a loop region at
   * the tip of the hairpin, which results in a slightly increase of the RMSD
