...
//	private String chainNGramBigger;
	 ...
	 * - good standard for substructure search
	 * - generally okay 
	 * 
	 */
	...
	public ScoreContainer scoreRefindeDoWhateveryouwantButGiveMeAScoreAndAnAlignment(
		...
		//residueToStringTransformer.get
...
//			
//			chainNGramBigger = chain1NGram_original;
//			chainNGramSmaller = chain2NGram_original;
//			 
//		} else {
//			chainBigger = chain2_original;
//			chainSmaller = chain1_original;
//			
//			chainNGramBigger = chain2NGram_original;
//			chainNGramSmaller = chain1NGram_original;
//		}
			...
			for (int j = 0; j < chainTargetIsTemplateInTMSCORE.size(); j++) {
				...
				if (bitSet.get(j)) {
					...
					//System.out.println("tempdistanceInnerCounterJ: " + tempDistanceInnerCounterOfJ);
							...
							//System.out.println("smallest: " + overallSmallestDistanceInOuterI);
...
//					double
//					 testDistance = Calc.getDistance(
//							chainQuery.get(i),
//							chainTarget.get(j));
//				
				
				//score only if ngrams are similar...
				// EXCEPT if they contain undefined results => then try to score anyway...
				//only score if they are similar...:
				
				// why? because if one compares very small pieces of say RNA
				// the ends are often undefined (eta theta == _ ) => so this
				// will match most likely to the end of another RNA => what can be wrong...
